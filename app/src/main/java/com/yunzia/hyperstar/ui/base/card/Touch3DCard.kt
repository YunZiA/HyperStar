package com.yunzia.hyperstar.ui.base.card
...
data class TiltAnimationState(
    val rotationX: Animatable<Float, AnimationVector1D>,
    val rotationY: Animatable<Float, AnimationVector1D>,
    val pivotX: Animatable<Float, AnimationVector1D>,
    val pivotY: Animatable<Float, AnimationVector1D>,
    ...
    internal fun updateTilt(
        ...
        coroutineScope.launch {
            launch { scale.animateTo(lerp(0.9f, 1f, distanceRatio), animationSpec) }
            launch { pivotX.animateTo(1 - relativeX, animationSpec) }
            launch { pivotY.animateTo(1 - relativeY, animationSpec) }
            launch { rotationX.animateTo(lerp(3f, -3f, relativeY), animationSpec) }
            launch { rotationY.animateTo(lerp(-3f, 3f, relativeX), animationSpec) }
        }
    }
...
fun Modifier.cutTouch(
    holdTimeThreshold: Long = 15L,
    moveThreshold: Float = 10f,
    stop: (Boolean) -> Unit = {},
    onPositionChanged: (Offset, Boolean) -> Unit = { _, _ -> } // 添加手指状态参数
): Modifier = composed {
    // 使用 remember 存储状态
    ...
    this.pointerInteropFilter { motionEvent ->
        ...
        when (motionEvent.actionMasked) {
            MotionEvent.ACTION_DOWN -> {
                ...
                stop(false)
                onPositionChanged(currentPosition, true)
                false
            }

            MotionEvent.ACTION_MOVE -> {
                initialPosition?.let { initial ->
                    val holdTime = System.currentTimeMillis() - startTime
                    val distance = (currentPosition - initial).getDistance()

                    if (holdTime > holdTimeThreshold && distance < moveThreshold) {
                        stop(true)
                    }
                }
            ...
            MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
                ...
                onPositionChanged(currentPosition, false) // false 表示手指抬起
                stop(false)
                false
            }
...
fun Modifier.withTiltEffect(
    ...
    moveThreshold: Float = 10f,          // 移动阈值，用于判断是否保持静止
    needCancel: MutableState<Boolean> = mutableStateOf(true),
    stop: (Boolean) -> Unit = {},
): Modifier = this
    .graphicsLayer {
        rotationX = tiltState.rotationX.value
    ...
    .pointerInput(Unit) {
        awaitPointerEventScope {
            while (true) {
                ...
                stop(false)
                tiltState.updateTilt(coroutineScope, downPosition, size)
...
fun rememberTiltAnimationState(): TiltAnimationState {
    ...
    return rememberSaveable(saver = TiltAnimationState.Saver) {
        TiltAnimationState(
            rotationX = Animatable(0f),
            rotationY = Animatable(0f),
            pivotX = Animatable(0f),
            pivotY = Animatable(0f),
            scale = Animatable(1f)
        )
    }
}
