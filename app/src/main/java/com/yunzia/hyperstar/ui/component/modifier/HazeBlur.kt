...
val LocalHazeState = compositionLocalOf<HazeState> { error("LocalHazeState not provided") }

fun Modifier.blur(hazeState: HazeState) = this.hazeSource(hazeState)
fun Modifier.blur(
    state: HazeState,
    zIndex: Float = 0f,
    key: Any? = null) = this.hazeSource(state,zIndex,key)

@OptIn(ExperimentalHazeApi::class)
@SuppressLint("UnnecessaryComposedModifier")
fun Modifier.showBlur(hazeState: HazeState,canDrawArea: ((HazeArea) -> Boolean)?) = composed {
