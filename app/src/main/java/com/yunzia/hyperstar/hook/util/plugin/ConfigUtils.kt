...
class ConfigUtils(private val classLoader: ClassLoader?) {
    ...
    val INSTANCE by lazy { configUtils.getStaticObjectField("INSTANCE") }

    fun textAppearanceChanged(configuration: Any?): Boolean{
        logD("INSTANCE = $INSTANCE")
        val res = INSTANCE.callMethod("textAppearanceChanged", configuration)
        StarLog.logD("res =  $res")
